...
--require('menu') -- main menu not used for game jam

function love.load()

    window = {translateX = 0, translateY = 0, scale = 1, width = 800, height = 600}

    -- initializeMainMenu() -- main menu not used for game jam submission

    gameState = 0 -- 0: normal, 1: rewinding, 2: win, 3: pause, 4: game over, 5: main menu, 6:level win animation, 7: game start
    freshGame = true -- require player to press mouse to focus the html game
    ...
    levelIndex = 1
    levelSizes = {{x=24,y=18},{x=24,y=18},{x=24,y=18},{x=24,y=18},{x=24,y=18}} --,{x=29,y=29},{x=33,y=48}}
...
function loadLevel()
    ...
    start_x = levels[levelIndex].startX
    start_y = levels[levelIndex].startY
end
...
function love.update(dt)
    ...
    if levelStartTimer > 0 then
        if freshGame == false then -- wait for mouse/tap to focus html game before starting
...
function love.keypressed(key, scancode, isrepeat)
    -- if key == "r" then -- reset, disabled for release
    --     gameState = 0
    --     newGame()
    -- else
    if key == "p" then
...
function love.draw()
    ...
    if gameState == 5 then
        ...
        if shakeDuration > 0 then -- screen shake
        ...
        if (mapHeight/32 <= 18) and (mapWidth/32 <= 25) then
            tx = 1
            ty = 5
        -- else -- handle levels larger than a single screen... not complete
        ...
        -- love.graphics.setColor(1, 0, 0) -- draw collision boxes for debugging
        ...
        if levelStartTimer > 0 and gameState ~= 4 and gameState ~= 6 then
            if freshGame then
                printOutlineFont("Click or tap to begin the adventure")
            else
